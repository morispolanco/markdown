...
def set_mirror_margins(section):
    """Activa márgenes simétricos para encuadernación."""
...
def add_page_number(footer):
    """Inserta el campo dinámico PAGE únicamente en el footer."""
...
def add_formatted_text(paragraph, text):
    """Procesa negritas y cursivas Markdown."""
...
# ==========================================
# 2. CONFIGURACIÓN EDITORIAL
# ==========================================

def setup_styles(doc):
    styles = doc.styles
    
    # Estilo Base: Garamond 11
    ...
    # Estilos de Título: Color automático (Negro) y Aptos
    for i in range(1, 6):
        style_name = f'Heading {i}'
        h = styles[style_name] if style_name in styles else styles.add_style(style_name, 1)
        h.font.name = 'Aptos'
        h.font.bold = True
        h.font.color.rgb = RGBColor(0, 0, 0) # Color automático/negro
...
def apply_layout(section, size_option, is_first=False):
    """Aplica dimensiones físicas a cada sección."""
    ...
    # Solo añadimos numeración si NO es la portada (is_first=False)
    if not is_first:
...
def run_book_conversion(md_text, meta, size_option):
    ...
    # Espacio para empujar autor/editorial
    ...
    # Aquí nos aseguramos de que el copyright sea el texto completo
    ...
    for line in lines:
        ...
        if header_match:
            ...
            if level == 1:
                # Salto a sección nueva en página impar para Capítulos
...
if 'meta' not in st.session_state:
    st.session_state.meta = {
        ...
        'publisher': "Editorial", 'year': "2026", 'copyright': ""
    }

with st.sidebar:
    st.header("1. Cargar Ficha")
    json_up = st.file_uploader("Ficha JSON", type=["json"])
    if json_up:
        try:
    ...
    m_copy = st.text_area("Copyright Completo", value=st.session_state.meta['copyright'], height=120)
    size_mode = st.selectbox("Tamaño", ["Trade Paperback (5.5x8.5)", "Estándar A4"])

st.header("2. Manuscrito")
md_file = st.file_uploader("Archivo Markdown", type=["md", "txt"])
content_raw = md_file.read().decode("utf-8") if md_file else ""
editor_content = st.text_area("Texto del Libro", value=content_raw, height=350)

if st.button("🚀 Generar Documento Maestro", use_container_width=True):
    if editor_content and m_title:
        bundle = {
            'title': m_title, 'subtitle': m_sub, 
            'author': m_author, 'publisher': m_pub, 
            'copyright': m_copy
        }
        with st.spinner("Compilando maquetación..."):
            docx_bytes = run_book_conversion(editor_content, bundle, size_mode)
            filename = f"{slugify(m_title)}.docx"
            st.download_button("📥 Descargar Libro Maquetado", docx_bytes, filename)
    else:
        st.error("Faltan datos obligatorios (Título o Contenido).")
